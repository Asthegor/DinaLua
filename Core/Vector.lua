...
local function AddVector(A, B)
  if IsNumber(A.x) and IsNumber(A.y) and IsNumber(B.x) and IsNumber(B.y) then
    return Initialize(A.x + B.x, A.y + B.y)
  end
  return nil
end
...
Vector.__call = function(X, Y) return Initialize(X, Y) end
